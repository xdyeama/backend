...
class LLMService:
    def __init__(self):
        ...
        self.main_prompt_text = """ I want you to act as you were a professional tour planner across Kazakhstan. Your goal now is to make and edit a unique tour plan for a journey across Kazakhstan and answer questions related to Kazakhstan. 
                                Suggest at 5 places either restaurants, museums, theaters, sport complexes, sightseeings based on the user cities of user choice using Google PLaces tool. Visit cities only mentioned in the input only once and strictly in order they were given. Strictly suggest 5 unique places for each day of a tour.
                                Give answer as a single JSON format where the key is the trip, which as a list of daily plans. Daily plans are the dictionaries which has the values of day_num which is the day number of the trip, the city where the trip is scheduled, activities, which is the list of activities planned for that day.Activities are the dictionaries which has the values of timestamp, which is the time of an activity, the place_name which is the name of the place, address, which the address of the place, place_description, which is the 3-4 sentences description of a place, coordinates, which is dictionary with keys lat for latitude of the place and lng for longitude of the place, website and contact number. Give empty string instead of null.
